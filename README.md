...
| **M3** — OnlyFansAPI sync: клики, подписки, spenders, revenue, snapshots | ✅ реализовано, для живых данных нужны ключ и Account ID в `.env` |
| **M6** — Деплой на VPS | ⏳ следующий шаг |
...
│   │   │   ├── activity.ts       # динамика по snapshot-ам
│   │   │   └── trends.ts         # сравнение периодов для KPI/трендов
│   │   └── of/                   # OnlyFansAPI client + scheduler
...
cd backend
cp .env.example .env
# проверь GOOGLE_CREDENTIALS_PATH, GLOSSARY_SHEET_ID и OnlyFansAPI env-переменные
...
- **Сводка**: суммарные клики / подписки / spenders / выручка / payout по выбранной модели
- **Динамика**: KPI сравниваются с прошлым периодом; доступны 7/30/90 дней и custom date range
- **Графики**: активность по дням, итоговые значения или дневной прирост
- **Drill-down** (клик по партнёру): карточка, список ссылок, фильтры, ставки CPF/RevShare и расчёт payout

Если OnlyFansAPI не настроен в `.env`, метрики **Clicks / Subs / Revenue / Spenders** не обновляются. После настройки ключа и Account ID они подтягиваются через manual sync или scheduler.
...
## Как работает OnlyFansAPI sync

Backend читает tracking links из OnlyFansAPI по каждому настроенному Creator Account ID, матчится с Glossary-ссылками по URL, обновляет `clicks_count / subscribers_count / spenders_count / revenue_total` в таблице `links` и пишет snapshot для истории.

Sync можно запускать вручную через `POST /api/sync`. Scheduler также запускает обновление автоматически раз в 5 часов, чтобы не сжигать API-кредиты слишком часто.
...
POST   /api/sync                            — форсированный OnlyFansAPI sync
GET    /api/sync/status                     — последние 5 sync-запусков + флаг конфигурации
GET    /api/activity                        — динамика по дням: days или start/end
GET    /api/trends                          — сравнение текущего периода с предыдущим
POST   /api/finance/sync                    — chargebacks / transactions / payouts
```
